...
{
	critical_section::scoped_lock lock(m_main->GetCriticalSection());
	GetDeviceResources()->SetLogicalSize(DirectX::XMFLOAT2(e->NewSize.Width, e->NewSize.Height));
	m_main->OnWindowSizeChanged();
}

std::shared_ptr<GraphicsEngine::DeviceResources> DirectXPage::GetDeviceResources()
{
	...
	{
		m_deviceResources = std::make_shared<GraphicsEngine::DeviceResources>();
		m_deviceResources->SetSwapChainPanel(std::make_unique<Converters::NativeSwapChainPanel>(swapChainPanel));
