...
void Renderer::CreateDeviceDependentResources()
{
	...
	m_rootSignatureManager.CreateDeviceDependentResources(textureCount);
	m_pipelineStateManager.CreateDeviceDependentResources(m_rootSignatureManager, textureCount);
...
bool Renderer::Render(const Common::Timer& timer)
{
	...
	PIXBeginEvent(commandList, PIX_COLOR(255, 0, 0), L"Begin G-Buffer Pass");
	{
	...
	PIXBeginEvent(commandList, PIX_COLOR(255, 0, 0), L"Draw");
	{
		m_pipelineStateManager.SetPipelineState(commandList, "GBufferPass-Opaque");
		m_scene->Render(timer, RenderLayer::Opaque);

		m_pipelineStateManager.SetPipelineState(commandList, "GBufferPass-Skinned");
		m_scene->Render(timer, RenderLayer::SkinnedOpaque);
	}
	PIXEndEvent(commandList);

	PIXBeginEvent(commandList, PIX_COLOR(255, 0, 0), L"Begin Lighting Pass");
	{
		// Set the graphics root signature:
...
CommandListManager& Renderer::GetCommandListManager()
{
