...
// Constructor
//------------------------------------------------------------------------------
Allocator::Allocator(size_t size, std::size_t objects, char* memory, const char* name) :
    ...
    m_name(name)
{
	...
	if (m_maxObjects)
	{
		...
		{
			m_pPool = static_cast<char*>(new char[m_blockSize * m_maxObjects]);
...
void* Allocator::Allocate(size_t size)
{
    ...
    if (!pBlock)
    {
        ...
        if (m_maxObjects)
        {
            ...
            {
                pBlock = static_cast<void*>(m_pPool + (m_poolIndex++ * m_blockSize));
            }
            else
            {
        ...
        {
        	m_blockCnt++;
            pBlock = static_cast<void*>(new char[m_blockSize]);
        }
    }
...
{
	auto pBlock = static_cast<Block*>(pMemory);
